function [sData] = mark_spindle2(sData, makeSpectrogramPlot)
...
signal = sData.(channelID).lfp;
sigma_filtsig = sData.(channelID).sigmaband;


% 
% zscore_signal = zscore(signal);
% noise_thres = 6*std(zscore_signal);
% zscore_signal(zscore_signal > noise_thres) = NaN;
% zscore_signal(zscore_signal < -noise_thres) = NaN;
% npnts = length(zscore_signal);
% srate = 2500;
% nyquist = srate/2;
...
% order_sigma = round( 50*srate/sigma_freq(1));
...
% filtpow_sigma2 = filt_sigma_pow(1:length(hz_sigma));
% 
% figure,
% plot(hz_sigma, filtpow_sigma2, 'b', 'linew', 2), hold on
...
% title(['Sigma (' num2str(sigma_freq(1)) '-' num2str(sigma_freq(2)) ' Hz)']);
% 
% 
...
% end
% 
% sigma_filtsig = filtfilt(filtkern_sigma, 1, ECOG);
...
ampl_env = abs(hilbert(sigma_filtsig));
ampl_evn2 = abs(hilbert(zscore(sigma_filtsig)));
% smooth amplitude envelope with a Gaussian filter of 200ms
smooth_ampl_env = smoothdata(ampl_evn2, 'Gaussian', 500);
...
for i = 1:length(spindleLocs)
    ...
    if lfpEndIndex > length(signal)
       lfpEndIndex = length(signal);
    end
    
    spindleSnips(i).lfp = signal(round(lfpStartIndex):round(lfpEndIndex));
    spindleIdx(i) = spindleLocs(i);
end

%% go to manual inspection of spindles and assign them to sData struct
[final_spindleLFP, final_spindleLocs, final_spindle_snip_loc] = inspectSpindles(spindleSnips,spindleCentralTimePoint,signal, spindleHalfLength,tempListSpindles ,makeSpectrogramPlot);
...
sData.(channelID).spindleCycl = nrOfCycles;
sData.(channelID).spindleDur = spindleDur;
sData.(channelID).spindleFreq = intraSpindleFreq;
sData.(channelID).spindleAmp = spindleAmp;
