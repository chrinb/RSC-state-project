...
% Preallocate
[significant_transients] = zeros( size(dff));

% Loop over ROIs
for roi_nr = 1:size(dff,1)
...
%     set(gca, 'xlim',[1 200], 'ylim',[-1 2])
    ...
    % Loop over significant transients
...
%         plot(frame_vec, 'linew',1)
    ...
    significant_transients(roi_nr,:) = frame_vec;
    
    roi_signal_sig_transients    = roi_signal_orig;
    roi_signal_nonsig_transients = roi_signal_orig;
...
%     clf
    ...
    sData.analysis.transients.roi_signal_sig_transients(roi_nr,:)    = roi_signal_sig_transients;
    sData.analysis.transients.roi_signal_nonsig_transients(roi_nr,:) = roi_signal_nonsig_transients;
end

%% Store output
sData.analysis.transients.significant_transients_logical = significant_transients;
sData.analysis.transients.standardized_dff               = dff_stand;

temp = sData.analysis.transients.roi_signal_sig_transients  ;
temp(isnan(temp)) = 0;

sData.analysis.transients.sig_transients = temp;
