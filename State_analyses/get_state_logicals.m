...
%     if isfield(sData.behavior, state_vec_name) && ~isempty(sData.behavior.(state_vec_name)) && ~(sum(sData.behavior.(state_vec_name)) == 0)
    ...
    %        corr_frame_index_stop  
        ...
        % Sometimes EventStartIdx is a column variable, other times a row variable
        % Find the index of the largest dimension to find out which
        % dimension to loop over
        [~, maxE_idx] = max( size(eventStartIdx));

        for i = 1:size(eventStartIdx, maxE_idx)
