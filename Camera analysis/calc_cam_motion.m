function sData = calc_cam_motion(sData)

% Get path to facemap output fole
...
facemap_path     = [mouse_folder, '\FaceCam'];
files = dir(fullfile(facemap_path, '*proc.mat'));
...
%% Load Facemap output
test = load(face_file_name)
files = dir(fullfile(facemap_path, '*.txt'));
...
if isempty(files)
    ...
    for i = 1:length(files)
        ...
        if contains(files(i).name, 'frametimes')
            ...
            cam_frames_path = fullfile(facemap_path, cam_file_name);
